...
using WindowsServiceWithWorkers.WinService;
    ...
    internal class Program
    {
        static int Main(string[] args)
        {
            if (Environment.UserInteractive)//Setup is executing or running in debug mode as an interactive console application
            {
                ...
                {
                    //Load service settings from ServiceSettings.xml file
                    var settings = ServiceSettings.LoadFromXmFile(Path.GetFullPath("ServiceSettings.xml"));

                    var myService = new SampleService(settings);
                    myService.Start();
